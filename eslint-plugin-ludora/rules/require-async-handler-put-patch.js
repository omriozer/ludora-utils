/**
 * @fileoverview Require async PUT/PATCH route handlers to be wrapped with asyncHandler
 * @author Ludora Team
 *
 * Split from main rule to allow incremental cleanup (PUT/PATCH handlers only)
 */
...
module.exports = {
  meta: {
    type: 'problem',
    docs: {
      description: 'Require async PUT/PATCH route handlers to be wrapped with asyncHandler',
      category: 'Best Practices',
      recommended: true
    },
    messages: {
      missingAsyncHandler: 'Async PUT/PATCH route handler must be wrapped with asyncHandler()'
    },
    schema: []
  },
  ...
  create(context) {
    ...
    function isPutPatchRouteHandler(node) {
      if (node.type !== 'CallExpression') return false;
      const callee = node.callee;
      if (callee.type === 'MemberExpression') {
        const object = callee.object;
        const property = callee.property;
        if (property.type === 'Identifier' && (property.name === 'put' || property.name === 'patch')) {
    ...
    return {
      CallExpression(node) {
        if (!isPutPatchRouteHandler(node)) return;
