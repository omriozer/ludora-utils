/**
 * @fileoverview Require async DELETE route handlers to be wrapped with asyncHandler
 * @author Ludora Team
 *
 * Split from main rule to allow incremental cleanup (DELETE handlers only)
 */
...
module.exports = {
  meta: {
    type: 'problem',
    docs: {
      description: 'Require async DELETE route handlers to be wrapped with asyncHandler',
      category: 'Best Practices',
      recommended: true
    },
    messages: {
      missingAsyncHandler: 'Async DELETE route handler must be wrapped with asyncHandler()'
    },
    schema: []
  },
  ...
  create(context) {
    ...
    function isDeleteRouteHandler(node) {
      if (node.type !== 'CallExpression') return false;
      const callee = node.callee;
      if (callee.type === 'MemberExpression') {
        const object = callee.object;
        const property = callee.property;
        if (property.type === 'Identifier' && property.name === 'delete') {
    ...
    return {
      CallExpression(node) {
        if (!isDeleteRouteHandler(node)) return;
