/**
 * @fileoverview Require async POST route handlers to be wrapped with asyncHandler
 * @author Ludora Team
 *
 * Split from main rule to allow incremental cleanup (POST handlers only)
 */
...
module.exports = {
  meta: {
    type: 'problem',
    docs: {
      description: 'Require async POST route handlers to be wrapped with asyncHandler',
      category: 'Best Practices',
      recommended: true
    },
    messages: {
      missingAsyncHandler: 'Async POST route handler must be wrapped with asyncHandler()'
    },
    schema: []
  },
  ...
  create(context) {
    ...
    function isPostRouteHandler(node) {
      if (node.type !== 'CallExpression') return false;
      const callee = node.callee;
      if (callee.type === 'MemberExpression') {
        const object = callee.object;
        const property = callee.property;
        if (property.type === 'Identifier' && property.name === 'post') {
    ...
    return {
      CallExpression(node) {
        if (!isPostRouteHandler(node)) return;
